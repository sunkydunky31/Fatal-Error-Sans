...
class MusicPlayer extends FlxGroup 
{
	...
	override function update(elapsed:Float)
	{
		...
		if (paused && !wasPlaying)
			songTxt.text = Language.getPhrase('musicplayer_playing', 'PLAYING: {1} (PAUSED)', [instance.songs[FreeplayState.curSelected].songName]);
		else
			songTxt.text = Language.getPhrase('musicplayer_playing', 'PLAYING: {1}', [instance.songs[FreeplayState.curSelected].songName]);
	...
	public function switchPlayMusic()
	{
		...
		if (playingMusic)
		{
			...
			[backend.InputFormatter.getKeyName(SPACE).toUpperCase(), backend.InputFormatter.getKeyName(CONTROL).toUpperCase(), 'R']);
			positionSong();
