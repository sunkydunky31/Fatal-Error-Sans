...
class AchievementPopup extends openfl.display.Sprite {
	...
	public function new(achieve:String, onFinish:Void->Void)
	{
		...
		if(achievement != null)
		{
			if(achievement.name != null) name = Language.getPhrase('achievement_${achievement.name}', achievement.name);
			if(achievement.description != null) desc = Language.getPhrase('description_${achievement.description}', achievement.description);
		}
