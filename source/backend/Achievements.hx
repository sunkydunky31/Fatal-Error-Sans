...
class Achievements {
	...
	public static function unlock(name:String, autoStartPopup:Bool = true):String {
		...
		{
			FlxG.sound.play(Paths.sound('Achieve'), 0.5);
			_lastUnlock = time;
		}
	...
	public static function addLuaCallbacks(lua:State)
	{
		Lua_helper.add_callback(lua, "getAchievementScore", function(name:String):Float
		{
			if(!achievements.exists(name))
			{
				FunkinLua.luaTrace('getAchievementScore: Couldn\'t find achievement: $name', false, false, FlxColor.RED);
				return -1;
			}
		...
		Lua_helper.add_callback(lua, "setAchievementScore", function(name:String, ?value:Float = 1, ?saveIfNotUnlocked:Bool = true):Float
		{
			if(!achievements.exists(name))
			{
				FunkinLua.luaTrace('setAchievementScore: Couldn\'t find achievement: $name', false, false, FlxColor.RED);
				return -1;
			}
		...
		Lua_helper.add_callback(lua, "addAchievementScore", function(name:String, ?value:Float = 1, ?saveIfNotUnlocked:Bool = true):Float
		{
			if(!achievements.exists(name))
			{
				FunkinLua.luaTrace('addAchievementScore: Couldn\'t find achievement: $name', false, false, FlxColor.RED);
				return -1;
			}
		...
		Lua_helper.add_callback(lua, "unlockAchievement", function(name:String):Dynamic
		{
			if(!achievements.exists(name))
			{
				FunkinLua.luaTrace('unlockAchievement: Couldn\'t find achievement: $name', false, false, FlxColor.RED);
				return null;
			}
			return unlock(name);
		});
		Lua_helper.add_callback(lua, "isAchievementUnlocked", function(name:String):Dynamic
		{
			if(!achievements.exists(name))
			{
				FunkinLua.luaTrace('isAchievementUnlocked: Couldn\'t find achievement: $name', false, false, FlxColor.RED);
				return null;
			}
