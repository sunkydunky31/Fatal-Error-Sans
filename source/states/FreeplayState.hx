...
class FreeplayState extends MusicBeatState {
	...
	override function create() {
		...
		DiscordClient.changePresence(Language.getPhrase('discordrpc_menu_freeplay', 'Freeplay Menu'), null);
		#end

		if (WeekData.weeksList.length < 1) {
			FlxTransitionableState.skipNextTransIn = true;
			persistentUpdate = false;
			MusicBeatState.switchState(new states.ErrorState("NO WEEKS ADDED FOR FREEPLAY\n\nPress {1} to go to the Week Editor Menu.\nPress {2} to return to Main Menu.",
				function() MusicBeatState.switchState(new states.editors.WeekEditorState()),
				function() MusicBeatState.switchState(new states.MainMenuState()), "freeplay_no_weeks"));
			return;
		}
		...
		var leText:String = Language.getPhrase("freeplay_tip",
			...
			[backend.InputFormatter.getKeyName(SPACE), "CTRL", "R"]);
		bottomString = leText;
		var size:Int = 16;
