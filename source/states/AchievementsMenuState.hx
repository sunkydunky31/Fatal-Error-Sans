...
class AchievementsMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		#if DISCORD_ALLOWED
		DiscordClient.changePresence(Language.getPhrase('discordrpc_menu_achievements', 'Achievements Menu'), null);
		#end
		...
		var menuBG:FlxSprite = new FlxSprite().loadGraphic(Paths.image('menuYellow'));
		menuBG.antialiasing = ClientPrefs.data.antialiasing;
		menuBG.setGraphicSize(Std.int(menuBG.width * 1.1));
		menuBG.updateHitbox();
		menuBG.screenCenter();
		menuBG.color = FlxColor.fromHSL(-174, 0, 7);
